...
//enum XXUnityVulkanResourceAccessMode
//{
//    // Does not imply any pipeline barriers, should only be used to query resource attributes
//    XXkUnityVulkanResourceAccess_ObserveOnly,

//    // Handles layout transition and barriers
//    XXkUnityVulkanResourceAccess_PipelineBarrier,

//    // Recreates the backing resource (VkBuffer/VkImage) but keeps the previous one alive if it's in use
//    XXkUnityVulkanResourceAccess_Recreate,
//};

//struct XXUnityVulkanImage
//{
//    XXUnityVulkanMemory memory;   // memory that backs the image
...
//struct XXUnityVulkanBuffer
//{
//    XXUnityVulkanMemory memory;   // memory that backs the buffer
...
struct XXUnityVulkanRecordingState
{
...
enum XXUnityVulkanGraphicsQueueAccess
{
    ...
    XXkUnityVulkanGraphicsQueueAccess_Allow,
};

enum XXUnityVulkanEventConfigFlagBits
{
    XXkUnityVulkanEventConfigFlag_EnsurePreviousFrameSubmission = (1 << 0), // default: set
    XXkUnityVulkanEventConfigFlag_FlushCommandBuffers = (1 << 1),           // submit existing command buffers, default: not set
    XXkUnityVulkanEventConfigFlag_SyncWorkerThreads = (1 << 2),             // wait for worker threads to finish, default: not set
    XXkUnityVulkanEventConfigFlag_ModifiesCommandBuffersState = (1 << 3),   // should be set when descriptor set bindings, vertex buffer bindings, etc are changed (default: set)
};

struct XXUnityVulkanPluginEventConfig
{
    XXUnityVulkanEventRenderPassPreCondition renderPassPrecondition;
    XXUnityVulkanGraphicsQueueAccess graphicsQueueAccess;
    uint32_t flags;
};
...
//enum XXUnityVulkanSwapchainMode
//{
...
//{
//    XXUnityVulkanSwapchainMode mode;
//};
...
UNITY_DECLARE_INTERFACE(IUnityGraphicsVulkan)
{
    ...
    void(UNITY_INTERFACE_API * ConfigureEvent)(int eventID, const XXUnityVulkanPluginEventConfig * pluginEventConfig);
    ...
    XXUnityVulkanInstance(UNITY_INTERFACE_API * Instance)();
    ...
    bool(UNITY_INTERFACE_API * CommandRecordingState)(XXUnityVulkanRecordingState * outCommandRecordingState, XXUnityVulkanGraphicsQueueAccess queueAccess);
    ...
    /***** UNUSED FUNCITON POINTER ******/
    //bool(UNITY_INTERFACE_API * AccessTexture)(void* nativeTexture, const VkImageSubresource * subResource, VkImageLayout layout,
    //    VkPipelineStageFlags pipelineStageFlags, VkAccessFlags accessFlags, XXUnityVulkanResourceAccessMode accessMode, XXUnityVulkanImage * outImage);
    ...
    /***** UNUSED FUNCITON POINTER ******/
    //bool(UNITY_INTERFACE_API * AccessBuffer)(void* nativeBuffer, VkPipelineStageFlags pipelineStageFlags, VkAccessFlags accessFlags, XXUnityVulkanResourceAccessMode accessMode, XXUnityVulkanBuffer * outBuffer);
