...
void VulkanExampleBase::setScreen(QScreen* targetScreen)
{
    screen = targetScreen;
}
#endif
...
//	}
    ...
    {
#endif
        // Paminder: Hack for real time Qtimer response
//        render();
//return;
		auto tStart = std::chrono::high_resolution_clock::now();
		if (viewUpdated)
...
#else
                window->setTitle(QString("QWindow") + QString(windowTitle.c_str()));
#endif
			}
			lastFPS = (float)frameCounter * (1000.0f / fpsTimer);
			frameCounter = 0;
...
#elif defined(VK_USE_PLATFORM_XCB_KHR)
// For Unity the surface are being catered by unity server.
#ifndef UNITY_BUILD
    initxcbConnection();
#endif
#endif
...
#endif
{
	...
	if (settings.fullscreen)
	{
		...
		height = destHeight = screen->height_in_pixels;
                ...
                width = destWidth = screenGeometry.width();
                height = destHeight = screenGeometry.height();
#endif
	}
...
#else
        window = new Window(this); // Create the window to provide display housing
	std::string windowTitle = getWindowTitle();
        window->setTitle(QString("QWindow: ") + QString(windowTitle.c_str()));
	if (settings.fullscreen)
	{
...
#ifndef USE_QWINDOW
        // For Qt Window: screen is setup from main() function via setScreen(..)
